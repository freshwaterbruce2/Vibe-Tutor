...
const FocusTimer: React.FC<FocusTimerProps> = ({ onSessionComplete }) => {
  ...
  const handleComplete = () => {
    ...
    if (mode === 'focus') {
      ...
      const currentPoints = Number(localStorage.getItem('studentPoints') || '0');
      localStorage.setItem('studentPoints', String(currentPoints + pointsEarned));
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-background-main via-background-main to-surface-dark">
      <div className="glass-card p-8 text-center space-y-6 max-w-md w-full">
        <div className="flex items-center justify-center gap-3">
          <Clock
            size={32}
            className={mode === 'focus' ? 'text-blue-400' : 'text-green-400'}
          />
        ...
        <div className="h-3 bg-surface-lighter rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${((mode === 'focus' ? 25 : 5) - minutes - seconds/60) / (mode === 'focus' ? 25 : 5) * 100}%`
            }}
          />
        </div>
        ...
        {mode === 'focus' && (
          <p className="text-sm text-text-secondary">
            Complete this session to earn {25} points
          </p>
        )}
