...
const GameSettings: React.FC<GameSettingsProps> = ({ config, onChange, gameType }) => {
                ...
                <div className="absolute top-full right-0 mt-2 w-80 glass-card p-6 rounded-xl border border-purple-500/50 shadow-2xl z-50">
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            <div className="text-left">
                                                <div className={`font-bold ${isSelected ? `text-transparent bg-clip-text bg-gradient-to-r ${info.color}` : 'text-white'}`}>
                                                    {info.label}
                                                </div>
                    ...
                    <div className="mb-6">
                        ...
                        <button
                            ...
                            className={`w-full p-3 rounded-lg border-2 transition-all ${config.timerMode === 'timed'
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <Clock size={20} className={config.timerMode === 'timed' ? 'text-cyan-400' : 'text-purple-400'} />
                                    <span className="text-white font-bold">
                                        {config.timerMode === 'timed' ? 'Timed Challenge' : 'Relaxed Mode'}
                                    </span>
                                </div>
                                <span className="text-xs text-gray-400">
                                    {config.timerMode === 'timed' ? `${Math.floor((config.timeLimit || 480) / 60)} min limit` : 'No time limit'}
                                </span>
                            </div>
                        </button>
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        <button
                            onClick={toggleHints}
                            className={`w-full p-3 rounded-lg border-2 transition-all ${config.hintsEnabled
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <Eye size={20} className={config.hintsEnabled ? 'text-yellow-400' : 'text-gray-400'} />
                                    <span className="text-white font-bold">
                    ...
                    <div className="mb-6">
                        ...
                        <button
                            onClick={toggleSound}
                            className={`w-full p-3 rounded-lg border-2 transition-all ${config.soundEnabled
                    ...
                    {/* High Contrast */}
                    <div>
                        <label className="block text-sm font-semibold text-gray-300 mb-3">Visual Accessibility</label>
                        <button
                            onClick={toggleHighContrast}
                            className={`w-full p-3 rounded-lg border-2 transition-all ${config.highContrast
                        ...
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <Eye size={20} className={config.highContrast ? 'text-orange-400' : 'text-gray-400'} />
                                    <span className="text-white font-bold">
...
export function getSavedGameConfig(gameType: string): GameConfig {
    try {
        const saved = localStorage.getItem(`game-config-${gameType}`);
        if (saved) {
            return { ...DEFAULT_GAME_CONFIG, ...JSON.parse(saved) };
        }
    } catch {
...
export function saveGameConfig(gameType: string, config: GameConfig): void {
    try {
        localStorage.setItem(`game-config-${gameType}`, JSON.stringify(config));
    } catch (error) {
