...
import { generateAnagrams, calculateAnagramScore } from '../services/puzzleGenerator';
...
const AnagramsGame: React.FC<AnagramsGameProps> = ({ subject, onComplete, onBack }) => {
  const [startTime] = useState(Date.now());
  const [anagrams, setAnagrams] = useState<AnagramChallenge[]>([]);
  ...
  useEffect(() => {
    ...
    const challenges = generateAnagrams(words);
    setAnagrams(challenges);
  }, [subject]);
    ...
    if (userAnswer.toUpperCase() === currentAnagram.original) {
      ...
      setTimeout(() => {
        if (currentIndex < anagrams.length - 1) {
          ...
          const result = calculateAnagramScore(solved.size + 1, anagrams.length, hintsUsed, timeSpent);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-pink-900 p-6 pb-36 md:pb-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="glass-card p-4 mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-white font-medium">{solved.size}/{anagrams.length}</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <div className="glass-card p-8">
          <h2 className="text-3xl font-bold text-center mb-6 neon-text-primary">{subject} Anagrams</h2>
          ...
          <div className="mb-8 text-center">
            <div className="inline-flex gap-2 mb-4">
              {currentAnagram.scrambled.split('').map((letter, i) => (
                <div
                  key={i}
                  className="w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center bg-gradient-to-br from-cyan-600 to-blue-600 rounded-lg text-white text-2xl sm:text-3xl font-bold shadow-lg"
                >
                  {letter}
                </div>
              ))}
          ...
          {showHint && (
            <div className="mb-6 p-4 bg-yellow-800/30 rounded-lg border border-yellow-500/30">
              ...
              <div className="text-white">
                First letter: <span className="font-bold text-xl">{currentAnagram.original[0]}</span>
              </div>
              <div className="text-white">
