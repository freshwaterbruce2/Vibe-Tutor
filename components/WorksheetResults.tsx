...
import type { WorksheetSession, DifficultyLevel } from '../types';
import { Star, Trophy, TrendingUp, RotateCcw, ArrowRight, Sparkles, Target } from 'lucide-react';
...
const WorksheetResults: React.FC<WorksheetResultsProps> = ({
  session,
  leveledUp,
  newDifficulty,
  starsToNextLevel,
  ...
  const { score, starsEarned, questions, answers } = session;
          ...
          <div className="mb-8 glass-card p-8 rounded-2xl border-2 border-yellow-500 bg-gradient-to-br from-yellow-500/20 to-orange-500/20 animate-[bounce_1s_ease-in-out_3]">
            <div className="text-center space-y-4">
              ...
              <p className="text-2xl text-white">
                You advanced to <span className="font-bold text-yellow-400">{newDifficulty}</span> level!
              </p>
            ...
            <div className="flex justify-center gap-2">
              {[1, 2, 3, 4, 5].map((num) => (
                ...
                >
                  <Star
                    size={48}
                    className={num <= (starsEarned || 0) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-600'}
                  />
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            <div className="text-center glass-card p-4 rounded-xl">
              ...
              <p className="text-2xl font-bold">
                {Math.floor((session.timeSpent || 0) / 60)}:{String((session.timeSpent || 0) % 60).padStart(2, '0')}
              </p>
            </div>
            <div className="text-center glass-card p-4 rounded-xl">
