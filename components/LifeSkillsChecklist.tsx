import { CheckCircle2, Circle, Home, Shirt, Sparkles, Utensils } from 'lucide-react';
...
const LIFE_SKILLS: LifeSkill[] = [
    ...
    { id: 'tomorrow-prep', category: 'evening', task: 'Prep for tomorrow (backpack/clothes)', icon: Shirt },
];
...
const LifeSkillsChecklist: React.FC<LifeSkillsChecklistProps> = ({ onTaskComplete }) => {
    const [completedToday, setCompletedToday] = useState<Set<string>>(new Set());
    const [selectedCategory, setSelectedCategory] = useState<'all' | 'morning' | 'chores' | 'hygiene' | 'evening'>('all');

    // Load from localStorage
    useEffect(() => {
        const saved = localStorage.getItem('lifeSkillsToday');
        const savedDate = localStorage.getItem('lifeSkillsDate');
        ...
        if (saved && savedDate === today) {
            ...
            localStorage.setItem('lifeSkillsDate', today);
        }
    }, []);

    // Save to localStorage
    useEffect(() => {
        localStorage.setItem('lifeSkillsToday', JSON.stringify(Array.from(completedToday)));
    }, [completedToday]);
        ...
        <div className="glass-card p-6 rounded-2xl border border-white/10">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-400">{completedToday.size} of {LIFE_SKILLS.length} completed ({completionRate}%)</p>
                </div>
                ...
                {filteredSkills.map(skill => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 p-3 rounded-lg transition-all ${isCompleted
                        ...
                        >
                            ...
                            <skill.icon className={`w-4 h-4 flex-shrink-0 ${isCompleted ? 'text-green-400' : 'text-gray-400'}`} />
                            <span className={`text-sm flex-1 text-left ${isCompleted ? 'text-green-300 line-through' : 'text-white'}`}>
                                {skill.task}
                            </span>
                        </button>
                    );
