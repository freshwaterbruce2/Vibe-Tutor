...
const HomeworkItem: React.FC<HomeworkItemProps> = ({ item, onToggleComplete, onBreakdownClick }) => {
  ...
  return (
    <div className={`glass-card p-6 rounded-2xl border relative overflow-hidden group transition-all duration-300 ${urgencyStyles[urgency]} ${item.completed
      ? 'opacity-60 scale-95'
      ...
      <div className="relative z-10 flex flex-col justify-between h-full min-h-[140px]">
        <div className="space-y-3">
          ...
          <h3 className={`text-lg font-semibold transition-all duration-300 ${item.completed
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-3.5 min-h-[46px] rounded-lg text-sm font-medium transition-all duration-300 ${item.completed
                ? 'glass-card text-[var(--text-muted)] hover:scale-105'
                : 'glass-button text-white hover:scale-105 shadow-lg'
                }`}
              style={{ touchAction: 'manipulation' }}
            >
              <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${item.completed
