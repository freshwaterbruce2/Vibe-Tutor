...
export function getCrosswordWords(subject: string, count: number): WordEntry[] {
  ...
  for (let i = 0; i < total; i++) {
    if (i % 3 === 0 && short.length > 0) {
      result.push(short.splice(Math.floor(Math.random() * short.length), 1)[0]);
    } else if (i % 3 === 1 && medium.length > 0) {
      result.push(medium.splice(Math.floor(Math.random() * medium.length), 1)[0]);
    } else if (long.length > 0) {
      result.push(long.splice(Math.floor(Math.random() * long.length), 1)[0]);
    } else {
