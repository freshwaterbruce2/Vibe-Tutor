...
export function getPlayerStats(): PlayerStats {
    const saved = localStorage.getItem('word-hunt-stats');
    if (saved) {
        try {
...
export function savePlayerStats(stats: PlayerStats): void {
    localStorage.setItem('word-hunt-stats', JSON.stringify(stats));
}

// Check and unlock badges
export function checkBadgeUnlocks(stats: PlayerStats, gameData: {
    wordsFound: number;
    timeSpent: number;
    hintsUsed: number;
    difficulty: string;
    ...
    stats.badges.forEach(badge => {
        ...
        switch (badge.id) {
            ...
            case 'no-hints-hero':
                ...
                break;
            case 'perfect-game':
                shouldUnlock = gameData.hintsUsed === 0 && gameData.timeSpent < 180 && gameData.allWordsFound;
                break;
            case 'streak-master':
            ...
            case 'hard-mode-champion':
                ...
                break;
            case 'no-time-waster':
                shouldUnlock = gameData.timeSpent < (gameData.difficulty === 'easy' ? 300 : gameData.difficulty === 'medium' ? 480 : 600);
                break;
        }
...
export function updateGameStats(
    ...
    const earnedXP = calculateXP(score, wordsFound === totalWords ? 3 : wordsFound > totalWords / 2 ? 2 : 1, difficulty);
