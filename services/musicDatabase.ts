...
class MusicDatabase {
  ...
  private sqlite = new CapacitorSQLite();
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      `);
            ...
            // Indexes
            await this.db.execute('CREATE INDEX IF NOT EXISTS idx_downloadUrl ON music_tracks(downloadUrl)');
            await this.db.execute('CREATE INDEX IF NOT EXISTS idx_lastPlayed ON music_tracks(lastPlayedAt)');

            console.log('[MusicDB] Connected to unified database');
    } catch (err) {
    ...
    async getAllTracks(): Promise<MusicTrackDB[]> {
        ...
        return (result.values as any[]).map(row => ({
            id: row.id,
            name: row.name,
            artist: row.artist,
