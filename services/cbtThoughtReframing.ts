...
export const THOUGHT_PATTERNS_INFO: Record<ThoughtPattern, {
  name: string;
  description: string;
  examples: string[];
  ...
  'all-or-nothing': {
    ...
    challengeQuestion: 'Is there a middle ground? What would be a more balanced way to think about this?',
  },
  'catastrophizing': {
...
export function identifyThoughtPattern(thought: string): ThoughtPattern[] {
  ...
  if (/\b(worst|terrible|awful|disaster|ruined|destroyed|doomed|hopeless)\b/.test(lowercaseThought)) {
    identifiedPatterns.push('catastrophizing');
  }

  // Mind reading (they think, everyone thinks, knows I'm, probably thinks)
  if (/\b(they think|everyone thinks|people think|knows? i'm|probably think|must think)\b/.test(lowercaseThought)) {
  ...
  if (/\b(i'm a|i'm such a|i am a|i'm so)\s+(idiot|failure|loser|stupid|dumb|worthless|broken|wrong)/.test(lowercaseThought)) {
 ...
 */
export function suggestReframes(automaticThought: string, patterns: ThoughtPattern[]): string[] {
  ...
  if (patterns.includes('all-or-nothing')) {
    ...
    suggestions.push('Look for the middle ground - not perfect, not terrible, but somewhere in between');
  }
...
export function saveThoughtEntry(entry: ThoughtEntry): void {
  try {
    ...
    localStorage.setItem('cbt_thought_journal', JSON.stringify(limited));
    console.log('[CBT] Saved thought entry');
  } catch (error) {
...
export function getThoughtHistory(): ThoughtEntry[] {
  try {
    const saved = localStorage.getItem('cbt_thought_journal');
...
export function getThoughtJournalStats() {
  ...
  const avgBefore = entriesWithReframe.length > 0
    ? entriesWithReframe.reduce((sum, e) => sum + e.emotionIntensity, 0) / entriesWithReframe.length
    : 0;

  const avgAfter = entriesWithReframe.length > 0
    ? entriesWithReframe.reduce((sum, e) => sum + (e.newEmotionIntensity || 0), 0) / entriesWithReframe.length
    : 0;
