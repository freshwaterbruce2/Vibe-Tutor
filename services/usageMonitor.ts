...
class UsageMonitor {
  ...
  private loadLimits(): UsageLimits {
    const stored = localStorage.getItem('usageLimits');
    if (stored) {
  ...
  private saveUsageData(): void {
    localStorage.setItem('usageData', JSON.stringify(this.usage));
  }

  private saveLimits(): void {
    localStorage.setItem('usageLimits', JSON.stringify(this.limits));
  }
