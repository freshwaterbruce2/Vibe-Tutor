...
class MediaSessionService {
  ...
  constructor() {
    ...
    if (this.isSupported) {
      console.log('✅ Media Session API supported - lock screen controls enabled');
    } else {
  ...
  public updatePlaybackState(state: 'none' | 'paused' | 'playing'): void {
    ...
    try {
      ...
      console.log('▶️ Media Session playback state:', state);
    } catch (error) {
  ...
  public updatePositionState(position: number, duration: number, playbackRate: number = 1.0): void {
    ...
    try {
      ...
      if (now - this.lastPositionUpdate > 5000) {
        console.log(`⏱️ Position: ${position.toFixed(1)}s / ${duration.toFixed(1)}s`);
