...
class AdaptiveAudioEngine {
  ...
  private duckedVolume: number = 1.0;
  ...
  private loadSettings(): void {
    try {
      const saved = localStorage.getItem('adaptive-audio-settings');
      if (saved) {
        const settings = JSON.parse(saved);
        this.state.volume = settings.volume ?? 0.5;
        this.state.currentScene = settings.lastScene ?? 'off';
      }
    } catch (error) {
  ...
  private saveSettings(): void {
    try {
      localStorage.setItem('adaptive-audio-settings', JSON.stringify({
   ...
   */
  async switchScene(scene: AudioScene, immediate: boolean = false): Promise<void> {
   ...
   */
  private async crossfade(newAudio: HTMLAudioElement, duration: number = 2000): Promise<void> {
   ...
   */
  duck(targetVolume: number = 0.2, duration: number = 500): void {
    ...
    this.fadeInterval = window.setInterval(() => {
      ...
      currentStep++;
      this.primaryAudio.volume = Math.max(targetVolume, this.duckedVolume - (volumeDelta * currentStep));
