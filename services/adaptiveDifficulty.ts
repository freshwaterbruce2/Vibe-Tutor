...
export function saveGamePerformance(performance: GamePerformance): void {
  try {
    ...
    console.log(`[AdaptiveDifficulty] Saved performance for ${performance.gameType}`);
  } catch (error) {
...
export function getGameHistory(gameType: string): GamePerformance[] {
  try {
    ...
    const history: GamePerformance[] = JSON.parse(saved);
    return history;
  } catch (error) {
...
export function resetGameHistory(gameType: string): void {
  try {
    ...
    console.log(`[AdaptiveDifficulty] Reset history for ${gameType}`);
  } catch (error) {
 ...
 */
export function getAllGameStats(): Record<string, {
  gamesPlayed: number;
  averageScore: number;
  currentDifficulty: DifficultyLevel;
