...
export function saveDailyEntry(entry: DailyAffirmationEntry): void {
  try {
    ...
    localStorage.setItem('daily_affirmations', JSON.stringify(limited));
    console.log('[Affirmations] Saved daily entry');
  } catch (error) {
...
export function getAffirmationHistory(): DailyAffirmationEntry[] {
  try {
    const saved = localStorage.getItem('daily_affirmations');
...
export function getAffirmationStats() {
  ...
  const recentMoods = history
    .filter(e => e.mood && e.date >= weekAgoStr)
  ...
  const previousMoods = history
    .filter(e => e.mood && e.date >= twoWeeksAgoStr && e.date < weekAgoStr)
    .map(e => moodToScore(e.mood!));
