...
const REALM_GAMES: Record<SubjectType, Array<{ id: string, name: string, desc: string, icon: ElementType, bgClass: string, iconClass: string }>> = {
  'Math': [
    { id: 'math', name: 'Math Adventure', desc: 'Solve math puzzles!', icon: Sparkles, bgClass: 'bg-fuchsia-500/20', iconClass: 'text-fuchsia-300' },
    { id: 'boss-math', name: 'The Math Menace', desc: 'Defeat the math boss!', icon: Flame, bgClass: 'bg-pink-500/20', iconClass: 'text-pink-300' }
  ],
  'Language Arts': [
    { id: 'anagrams', name: 'Anagrams', desc: 'Unscramble words!', icon: Puzzle, bgClass: 'bg-violet-500/20', iconClass: 'text-violet-300' },
    { id: 'crossword', name: 'Crossword', desc: 'Fill in the clues!', icon: Sparkles, bgClass: 'bg-rose-500/20', iconClass: 'text-rose-300' },
    { id: 'wordbuilder', name: 'Word Builder', desc: 'Build words letter by letter!', icon: Sparkles, bgClass: 'bg-purple-500/20', iconClass: 'text-purple-300' }
  ],
  'Science': [
    { id: 'boss-science', name: 'The Mad Scientist', desc: 'Test your science knowledge!', icon: Flame, bgClass: 'bg-fuchsia-500/20', iconClass: 'text-fuchsia-300' }
  ],
  'History': [
    { id: 'boss-history', name: 'The Time Bandit', desc: 'A historical showdown!', icon: Flame, bgClass: 'bg-pink-500/20', iconClass: 'text-pink-300' }
  ],
  'Bible': []
};
  ...
  return (
    <div className="vibe-page-shell min-h-screen p-4 md:p-8 pb-36 font-sans bg-[var(--background-main)]">
       <div className="max-w-4xl mx-auto">
          <button onClick={onBack} className="mb-6 flex items-center gap-2 text-[var(--text-secondary)] hover:text-white transition-colors">
          ...
          <div className="glass-card p-8 rounded-3xl mb-8 border-[1px] border-[var(--glass-border)] text-center shadow-lg relative overflow-hidden">
             ...
             <Trophy size={48} className="mx-auto text-[var(--secondary-accent)] mb-4 animate-bounce relative z-10" />
             <h1 className="vibe-hero-title text-4xl md:text-5xl font-black mb-4 relative z-10">
                The {subject} Realm
             </h1>
             <p className="text-[var(--text-secondary)] text-lg mb-8 relative z-10">Embark on an epic academic quest through the realm!</p>
             
             <button onClick={() => onStartWorksheet(subject)} className="relative z-10 vibe-cta w-full md:w-auto md:px-12 px-6 py-4 rounded-2xl font-black text-xl inline-flex items-center justify-center gap-3 active:scale-95 transition-all text-white group">
          ...
          {games.length > 0 && (
             <div>
               <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                 <Sparkles className="text-[var(--primary-accent)]" /> Side Quests
               </h2>
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {games.map(game => {
                    const Icon = game.icon;
                    return (
                      <button key={game.id} onClick={() => setActiveGame(game.id)} className="glass-card p-6 rounded-2xl border-[1px] border-[var(--glass-border)] hover:border-[var(--secondary-accent)]/60 text-left transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_10px_40px_-10px_rgba(244,114,182,0.28)] group flex flex-col h-full bg-[var(--glass-surface)] backdrop-blur-md">
                         ...
                         <p className="text-[var(--text-secondary)] text-sm flex-grow leading-relaxed">{game.desc}</p>
                      </button>
                    )
                  })}
               </div>
